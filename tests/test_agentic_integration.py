...
async def test_agent_step_attribution_preserved_through_chain() -> None:
    ...
    await planner.execute({"user_query": "test"})
    assert any(c.get("agent_step") == "query_planner" for c in planner_calls)
    ...
    await critic.execute({"draft": "draft answer", "user_query": "test"})
    assert any(c.get("agent_step") == "critic" for c in critic_calls)

    synth_svc, synth_calls = _make_llm_service("synthesized answer")
    synth = SynthesizerAgent(llm_service=synth_svc, prompt_manager=pm)
    await synth.execute({"sources": ["source1"], "user_query": "test"})
    assert any(c.get("agent_step") == "synthesizer" for c in synth_calls)
