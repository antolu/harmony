...
from harmony.api.services.admin import OllamaHostService
...
@pytest.fixture
def service(repo: MagicMock, model_repo: MagicMock) -> OllamaHostService:
    svc = OllamaHostService()
    svc._repo = repo
...
def _host_row(host_id: str = "host-1") -> OllamaHostRow:
    now = datetime.datetime.now(tz=datetime.UTC)
    return OllamaHostRow(
        id=host_id,
        name="local",
        url="http://x",
        host_type="ollama",
...
@pytest.mark.asyncio
async def test_create_rejects_invalid_host_type(service: OllamaHostService) -> None:
...
@pytest.mark.asyncio
async def test_update_rejects_invalid_host_type(service: OllamaHostService) -> None:
    with pytest.raises(ValueError, match="Invalid host_type"):
...
async def test_delete_blocks_when_models_use_host_and_force_false(
    service: OllamaHostService, repo: MagicMock, model_repo: MagicMock
) -> None:
